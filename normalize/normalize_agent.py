...
def validation_is_passed(
    ...
    """
    Check Validation Agent result.

    Preferred source:
        data/validation/<same filename>

    Backward compatibility:
        If no validation report is supplied, also support older
        Discovery JSON files that stored validation status inline.
    """
    if isinstance(validation_result, dict):
        status = clean_text(
            validation_result.get("status")
            or validation_result.get("validation_status")
        ).upper()
...
def validation_report_path(input_path: Path) -> Path:
    """Return the matching Validation Agent report path."""
    return VALIDATION_DIR / input_path.name


def load_validation_result(
    input_path: Path,
) -> tuple[dict[str, Any] | None, str | None]:
    """Load the Validation Agent report for the same Discovery filename."""
    report_path = validation_report_path(input_path)

    if not report_path.exists():
        return None, f"validation report not found: {report_path}"

    try:
        report = json.loads(report_path.read_text(encoding="utf-8"))
    except Exception as exc:
        return None, f"validation report read failed: {exc}"

    if not isinstance(report, dict):
        return None, "validation report root is not an object"
...
def normalize_file(
    ...
    # Validation Agent writes a separate report using the same filename.
    # Example:
    #   data/discovery/0005_contract.json
    #   data/validation/0005_contract.json
    validation_result, validation_error = load_validation_result(input_path)

    if validation_error:
        return False, validation_error

    if not validation_is_passed(data, validation_result):
        ...
        return (
            False,
            f"validation status is not 'PASSED' (got '{validation_status or 'UNKNOWN'}')",
        )
