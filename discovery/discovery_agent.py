...
#             """() => { const e=document.scrollingElement||document.documentElement; return {scrollHeight:e.scrollHeight, clientHeight:e.clientHeight}; }"""
#         )
...
#                     continue
#                 m = el.evaluate(
#                     """e => ({scrollHeight:e.scrollHeight, clientHeight:e.clientHeight})"""
#                 )
...
#                 if kind == "document":
#                     m = page.evaluate(
#                         """() => { const e=document.scrollingElement||document.documentElement; return {top:e.scrollTop, height:e.scrollHeight, viewport:e.clientHeight}; }"""
#                     )
#                 else:
#                     m = target.evaluate(
#                         """e => ({top:e.scrollTop, height:e.scrollHeight, viewport:e.clientHeight})"""
#                     )
#                 top = m.get("top", 0)
#                 maximum = max(0, m.get("height", 0) - m.get("viewport", 0))
#                 if top == last:
#                     stable += 1
#                 else:
#                     stable = 0
...
#                 if kind == "document":
#                     page.evaluate(
#                         """y => { const e=document.scrollingElement||document.documentElement; e.scrollTo(0,y); }""",
#                         (
#                             maximum
#                             if top >= maximum - 5
#                             else min(
#                                 maximum,
#                                 top + max(300, int(m.get("viewport", 700) * 0.75)),
#                             )
#                         ),
#                     )
#                 else:
...
#                         if top >= maximum - 5
#                         else min(
#                             maximum, top + max(300, int(m.get("viewport", 700) * 0.75))
#                         )
#                     )
...
#                 item = locator.nth(i)
#                 try:
#                     if item.is_visible() and normalize_text(
#                         item.inner_text()
...
#             {action for field in data["fields"] for action in field.get("actions", [])}
#         ),
#         "business_buttons": [
#             button.get("text", "") for button in data["buttons"] if button.get("text")
#         ],
...
#                     "+ new",
#                     "add",
#                 } and not is_workspace_ui(text=text):
#                     new_match = True
#                     break
#             except Exception:
#                 continue
...
#             if is_notification_element(text=candidate) or is_workspace_ui(
#                 text=candidate
#             ):
#                 continue
#             if low in {
...
#                 }
#             )
#             log(f"MODULE DOCTYPE FOUND: {candidate}" + (f" -> {href}" if href else ""))
#             return
...
#             # ============================================================
#             # IMPORTANT: ERPNext shortcut widgets contain extra UI text
#             # such as "0 Open", "15 Assigned", etc.
#             # Read the real DocType from widget-title / title attribute.
#             # Example:
#             #   <span class="ellipsis" title="Lead">Lead</span>
#             # The Global Search term will later become "New Lead".
#             # ============================================================
#             widget_name = ""
#             title_nodes = item.locator(
#                 ".widget-title span[title], " ".widget-title .ellipsis"
#             )
#             try:
#                 title_count = min(title_nodes.count(), 10)
#             except Exception:
#                 title_count = 0

#             for title_index in range(title_count):
#                 node = title_nodes.nth(title_index)
#                 try:
#                     if not node.is_visible():
#                         continue
#                     widget_name = clean_text(
#                         node.get_attribute("title") or node.inner_text()
#                     )
#                     if widget_name:
#                         break
#                 except Exception:
...
#             # aria-label is also reliable for shortcut widgets:
#             # <div ... aria-label="Lead">
#             if not widget_name:
#                 widget_name = clean_text(aria)

#             if widget_name:
#                 add_candidate(
#                     widget_name,
#                     href=href,
#                     source="module_right_side_widget_title",
#                 )
#             else:
#                 # Non-widget cards can still use their text/heading.
#                 add_candidate(
#                     text_value or title,
#                     href=href,
...
#                 add_candidate(
#                     data_doctype or widget_name or text_value,
#                     href=href,
...
#                 if "/app/" in page.url and validate_doctype_list_page(
#                     page, main_doctype
#                 ):
...
#     if not is_doctype_list_route(route):
#         raise RuntimeError(f"Unsafe DocType route: {doctype} -> {route}")
...
#         raise RuntimeError(f"DocType route returned HTTP {response.status}: {route}")

#     if not validate_doctype_list_page(page, doctype):
#         raise RuntimeError(f"Route is not a confirmed DocType list: {doctype}")
...
#     if not result:
#         raise RuntimeError(f"DocType could not be opened from Global Search: {doctype}")
...
#                     if is_system_ui(text=doctype) or is_notification_element(
#                         text=doctype
#                     ):
...
def clean_doctype_name(name):
    value = clean_text(name)
    return value[4:].strip() if value.lower().startswith("new ") else value


def build_doctype_search_name(doctype):
    doctype = clean_doctype_name(doctype)
...
def discover_module_docs(page):
    """Collect the real DocType title from module shortcut cards.

    Example: Lead / 0 Open -> Lead only.
    """
    docs = []
    seen = set()

    def add(name, href="", source="module"):
        name = clean_doctype_name(clean_text(name))
        if not name or is_system_ui(text=name) or is_notification_element(text=name):
            return
        key = normalize_text(name)
        if key in seen:
            return
        seen.add(key)
        docs.append({"doctype": name, "href": href, "source": source})
        log(f"MODULE DOCTYPE FOUND: {name} | search={build_doctype_search_name(name)}")
    ...
    for sel in [
        ...
        if scope:
            break
    scope = scope or page.locator("body")

    cards = scope.locator(
        ".shortcut-widget-box, .widget, .link-card, .module-card, .desk-card, a, button, [role='button']"
    )
    try:
        count = min(cards.count(), 2500)
    except Exception:
        count = 0
    for i in range(count):
        item = cards.nth(i)
        try:
            ...
            title = clean_text(item.get_attribute("title") or "")
            href = item.get_attribute("href") or ""
            if is_notification_element(text=aria, title=title) or is_workspace_ui(
                text=aria, title=title
            ):
                continue
            widget = item.locator(
                ".widget-title span[title], .widget-title .ellipsis"
            ...
            try:
                if widget.count() > 0 and widget.is_visible():
                    name = clean_text(
                        widget.get_attribute("title") or widget.inner_text()
                    )
            except Exception:
                pass
            add(
                name or aria or title or clean_text(item.inner_text()),
                href,
                "module_widget",
            )
        except Exception:
    ...
    # Non-clickable widget text fallback: extract only title before known counter/status.
    try:
        body = scope.inner_text()
    except Exception:
        body = ""
    for line in [clean_text(x) for x in body.splitlines() if clean_text(x)]:
        m = re.match(
            r"^(.*?)\s+\d+\s+(Open|Assigned|Overdue|Draft|Pending)$", line, re.I
        )
        if m:
            add(m.group(1), source="module_text_counter")
...
def is_blank_new_form(page, doctype):
    path = urlparse(page.url).path.lower() if page.url else ""
    if "new-" in path or "/new/" in path:
        return True
    try:
        body = normalize_text(page.locator("body").inner_text(timeout=5000))
        return (
            normalize_text(doctype) in body
            and page.locator("[data-fieldname]").count() > 0
            and ("save" in body or "submit" in body)
        )
    except Exception:
        return False
...
def search_doctype_from_home(page, doctype):
    main_doctype = clean_doctype_name(doctype)
    open_home(page)
    search = find_global_search_box(page)
    if not search:
        raise RuntimeError("Global search box not found.")
    search_name = build_doctype_search_name(main_doctype)
    log(f"GLOBAL SEARCH: {search_name}")
    search.click()
    ...
    for selector in [
        ...
        for i in range(count):
            ...
            try:
                ...
                if (
                    not raw
                    or is_notification_element(text=raw)
                    or is_workspace_ui(text=raw)
                ):
                    continue
                result_name = clean_doctype_name(raw)
                if normalize_text(result_name) != normalize_text(main_doctype):
                    continue
                href = item.get_attribute("href") or ""
                if href and not is_doctype_list_route(href):
                    continue
                log(f"SEARCH RESULT FOUND: {search_name} -> {raw}")
                item.click()
                page.wait_for_timeout(WAIT_LONG)
                if is_blank_new_form(page, main_doctype):
                    return page.url
                if "/app/" in page.url and validate_doctype_list_page(
                    page, main_doctype
                ):
                ...
                continue
    log(f"SEARCH RESULT NOT FOUND: {search_name}")
    return None


def open_doctype_list(page, doctype):
    main_doctype = clean_doctype_name(doctype)
    result = search_doctype_from_home(page, main_doctype)
    if not result:
        raise RuntimeError(
            f"DocType could not be opened from Global Search: {main_doctype}"
        )
    return result


def find_add_button(page, doctype):
    """Find ONLY the exact New action on the already validated DocType list."""
    if not is_doctype_list_route(page.url):
        return None
    buttons = page.locator("button, a, [role='button']")
    try:
        count = min(buttons.count(), 1000)
    except Exception:
        count = 0
    for i in range(count):
        item = buttons.nth(i)
        try:
            ...
            if normalize_text(raw) != "new":
                continue
            if is_notification_element(text=raw) or is_workspace_ui(text=raw):
                continue
            return item
        except Exception:
            continue
...
def parse_args():
    ...
    parser.add_argument(
        ...
        nargs="+",
        required=False,
        help=("Discover one or more DocTypes serially."),
    )
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            if args.doctype:
                ...
                total = len(args.doctype)
                for index, raw in enumerate(args.doctype, start=1):
                    doctype = clean_doctype_name(raw)
                    log(f"DOCTYPE {index}/{total}: {doctype}")
                    try:
                        open_home(page)
                        discover_doctype(page, doctype, module_name="")
                    except Exception as exc:
                        log(f"FAILED: {doctype} -> {exc}")
                    finally:
                        try:
                            open_home(page)
                        except Exception:
