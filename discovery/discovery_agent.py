...
def discover_all_form_tabs(page):
    ...
    if not tabs:
        snapshot = _collect_current_form_snapshot(page, active_tab="")
        return [], snapshot
    ...
    for tab_index, tab_info in enumerate(tabs):
        ...
        snapshot = _collect_current_form_snapshot(page, active_tab=label)
...
def discover_module_docs(page):
    """
    Discover ONLY business DocTypes exposed by the opened module's right side.

    Required flow:
        Login -> Home -> left Module -> right-side module content
        -> identify DocType -> open its list -> New -> read blank form
        -> return to module -> next DocType.

    This function ONLY collects candidates. It never opens a business record
    and never clicks a candidate. A candidate must have a real /app/<slug>
    list route; record routes and system UI are rejected.
    """
    docs = []
    seen = set()
    scope = _module_content_scope(page)

    clickables = scope.locator("a, button, [role='button'], .module-card, .desk-card, .link-card, .widget")
    try:
        count = min(clickables.count(), 2000)
    except Exception:
        count = 0
    ...
    for i in range(count):
        ...
        try:
            ...
            if is_notification_element(text=text, aria_label=aria, title=title):
                continue
            if is_workspace_ui(text=text, aria_label=aria, title=title):
                continue

            href = _extract_route_from_clickable(item)
            if not href or not is_doctype_list_route(href):
                continue

            # Use the href slug as a fallback name when the card text is noisy.
            candidate = clean_doctype_candidate(text)
            if not candidate:
                parsed = urlparse(urljoin(ERP_URL + "/", href))
                parts = [part for part in parsed.path.split("/") if part]
                candidate = parts[-1].replace("-", " ").title() if len(parts) == 2 else ""
                candidate = clean_doctype_candidate(candidate)
            ...
            docs.append({
                "doctype": candidate,
                "href": href,
                "source": "module_right_side",
            })
            log(f"MODULE DOCTYPE FOUND: {candidate} -> {href}")

        except Exception as exc:
            log(f"SKIP MODULE CANDIDATE: {exc}")

    log(f"Module DocType links collected: {len(docs)}")
    return docs
...
def search_doctype_from_home(page, doctype):
    """
    Direct DocType discovery navigation.

    FLOW:
        Login -> Home -> Global Search -> exact DocType result
        -> DocType List -> target-specific New Document

    The search box is navigation only. It is never saved as a business
    DocType and no notification/bell/workspace item is clicked.
    """
    ...
    if is_system_ui(text=doctype) or is_notification_element(text=doctype):
        log(f"BLOCKED SEARCH TARGET (SYSTEM UI): {doctype}")
        return None

    # ------------------------------------------------------------
    # ALWAYS START FROM ERPNext HOME
    # ------------------------------------------------------------
    open_home(page)
    ...
    log(f"GLOBAL SEARCH: {doctype}")

    try:
        search_box.click()
        search_box.fill(doctype)
    ...
    for selector in result_selectors:
        ...
        for i in range(count):
            ...
            try:
                ...
                # Search results may contain record counters. Compare the
                # cleaned label against the requested DocType.
                cleaned_result = clean_doctype_candidate(text)

                if normalize_text(cleaned_result) != target:
                    continue

                # If a route is present, it MUST be a DocType list route.
                if href and not is_doctype_list_route(href):
                    continue

                log(f"EXACT SEARCH RESULT FOUND: {doctype}")

                # ------------------------------------------------
                # CLICK ONLY THE EXACT BUSINESS RESULT
                # ------------------------------------------------
                item.click()
                page.wait_for_timeout(WAIT_LONG)

                # If result itself has a route, use it safely when the UI
                # click opens something unexpected.
                ...
                if "/app/" in page.url:
                    if validate_doctype_list_page(page, doctype):
                        log(f"DocType list opened: {page.url}")
                        return page.url

            except Exception:
                continue

    log(f"EXACT SEARCH RESULT NOT FOUND: {doctype}")
    return None
...
def open_doctype_list(page, doctype, href=""):
    ...
    navigation path.
    """
    log(f"SEARCHING DOCTYPE BY NAME: {doctype}")
...
def discover_doctype(
    ...
    data["discovery_notes"] = {
        "navigation": (
            "Login -> ERPNext Home -> Global Search by exact DocType name -> DocType List "
            "-> New <DocType> (or validated New) -> blank document -> read all fields -> back to Module"
        ),
        "doctype_validation": (
